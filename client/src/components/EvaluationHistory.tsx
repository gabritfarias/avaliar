...
import { Evaluation, User as UserType } from '../types';
...
export const EvaluationHistory: React.FC<EvaluationHistoryProps> = ({
  ...
  const availableStores = useMemo(() => {
    ...
    evaluations.forEach((ev) => {
      if (ev.store?.id && ev.store?.name) {
        map.set(ev.store.id, ev.store.name);
      }
    });
  ...
  const filteredEvaluations = evaluations.filter((ev) => {
    if (user?.role === 'MASTER' && selectedStoreFilter !== 'ALL') {
      if (ev.storeId !== selectedStoreFilter) return false;
    }
    ...
    const storeName = (ev.store?.name || '').toLowerCase();
            ...
            <div className="flex items-center gap-2">
              ...
              {user?.role === 'MASTER' ? (
                <span className="bg-purple-100 text-purple-800 text-[10px] font-bold px-2 py-0.5 rounded-full border border-purple-200 flex items-center gap-1">
                  <Crown className="w-3 h-3" /> Visão Geral Master
                </span>
              ) : (
                <span className="bg-emerald-100 text-emerald-800 text-[10px] font-bold px-2 py-0.5 rounded-full border border-emerald-200 flex items-center gap-1">
                  <Store className="w-3 h-3" /> {user?.name || 'Sua Loja'}
                </span>
              )}
            </div>
            <p className="text-xs text-slate-500 mt-0.5">
              {user?.role === 'MASTER'
                ? 'Visualização consolidada de todas as avaliações com filtro por loja.'
                : 'Exibindo avaliações realizadas exclusivamente pela sua unidade.'}
            </p>
          </div>

          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
            {/* Seletor de Loja (Apenas para Master) */}
            {user?.role === 'MASTER' && availableStores.length > 0 && (
              <div className="relative min-w-[180px]">
                <select
                  value={selectedStoreFilter}
                  onChange={(e) => setSelectedStoreFilter(e.target.value)}
                  className="w-full min-h-[44px] pl-3 pr-8 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 cursor-pointer"
                >
                  <option value="ALL">🏢 Todas as Lojas ({evaluations.length})</option>
            ...
            <div className="relative w-full sm:w-72">
              ...
              <input
                ...
                placeholder="Buscar modelo, cliente, loja..."
                className="w-full min-h-[44px] pl-9 pr-8 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500"
              />
              {searchTerm && (
                <button
                  onClick={() => setSearchTerm('')}
                  className="w-8 h-8 absolute right-1 top-1.5 flex items-center justify-center text-xs text-slate-400 hover:text-slate-600 cursor-pointer"
                >
                  ✕
                </button>
              )}
              ...
              >
                ...
                <div className="flex items-center justify-between pb-2 border-b border-slate-100">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    {user?.role === 'MASTER' && ev.store?.name && (
                      <span className="inline-flex items-center gap-1 text-[10px] font-bold bg-purple-50 text-purple-800 px-1.5 py-0.5 rounded border border-purple-200">
              ...
              <table className="w-full text-left text-xs sm:text-sm">
                <thead className="bg-slate-50 text-slate-500 uppercase tracking-wider text-[10px] sm:text-xs font-bold border-b border-slate-200">
                  <tr>
                    ...
                    {user?.role === 'MASTER' && <th className="p-4">Loja / Unidade</th>}
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredEvaluations.map((ev) => (
                    <tr key={ev.id} className="hover:bg-slate-50/70 transition">
                      ...
                      {user?.role === 'MASTER' && (
                        <td className="p-4 whitespace-nowrap">
                          <span className="inline-flex items-center gap-1.5 text-xs font-bold bg-purple-50 text-purple-900 px-2.5 py-1 rounded-lg border border-purple-200">
                            <Store className="w-3.5 h-3.5 text-purple-600" />
                            <span>{ev.store?.name || 'Loja Matriz'}</span>
                          </span>
                        </td>
                      )}
