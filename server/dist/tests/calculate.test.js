...
async function runTests() {
    ...
    try {
        ...
        await prisma_1.prisma.model.deleteMany({ where: { name: 'iPhone Teste Especial' } });
        // 7.1 Criação do modelo com capacidades e peças padrão
        const createdModel = await prisma_1.prisma.model.create({
            data: {
        ...
        const historyEval = await prisma_1.prisma.evaluation.create({
            data: {
        ...
        for (const v of createdModel.variants) {
            await prisma_1.prisma.evaluation.updateMany({
        ...
        await prisma_1.prisma.model.delete({ where: { id: createdModel.id } });
        // 7.4 Verificar que as peças e variantes do modelo foram excluídas em cascata
        const remainingVariants = await prisma_1.prisma.variant.findMany({ where: { modelId: createdModel.id } });
        const remainingParts = await prisma_1.prisma.part.findMany({ where: { modelId: createdModel.id } });
        ...
        const preservedEval = await prisma_1.prisma.evaluation.findUnique({ where: { id: historyEval.id } });
        ...
        await prisma_1.prisma.evaluation.delete({ where: { id: historyEval.id } });
